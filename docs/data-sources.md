...
**Дыры после БВИ:** после блока БВИ встречаются строки с низким баллом без галки БВИ (отозванные и т.п.). Их пропускаем; остановка по `min_fetch_score` — только после первого балла `>=` порога.
### Направления (seed)
...
**Олимпиадники (БВИ):** абитуриенты с `noExam: true` в начале списка всегда сохраняются целиком, даже если `total` меньше порога (часто 5–10 баллов за индивидуальные достижения). Остановка по порогу — на первом абитуриенте без `noExam` с баллом `> 0` и `< min_fetch_score`.

Таймаут запроса шага 4 увеличен до 120 с (`timeout` в api_config) — ответ может быть большим.
